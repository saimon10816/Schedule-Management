                            ...
                            <div class="card-header">
                                ...
                                @if (auth()->check())
                                    @if (auth()->user()->isAdmin())
                                        <a href="{{route('admin.companyB.create')}}" class="btn btn-primary">Add</a>
                                    @endif
                                @endif
                            </div>
                            {{--                                @if (auth()->check())--}}
                            {{--                                    @if (auth()->user()->isAdmin())--}}
                            {{--                                        <a href="{{route('admin.companyB.create')}}"--}}
                                ...
                                <div class="tab-pane fade active show" id="st_matrics-today" role=""
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-social">
                                            ...
                                            @if(count($companyBlists) > 0)
                                                @foreach($companyBlists as $companyBlist)
                                                    <tr>
                                                        <td>
                                                            {{$companyBlist->id}}
                                                        </td>
                                                        <td>
                                                            <a href="">{{$companyBlist->date}}</a>
                                                        </td>
                                                        <td>{{$companyBlist->postSite}}</td>
                                                        <td>{{$companyBlist->guardName}}</td>
                                                        <td>{{$companyBlist->shiftStart}}</td>
                                                        <td>{{$companyBlist->shiftEnd}}</td>
                                                        <td>{{$companyBlist->totalHour}}</td>
                                                        <td>{{$companyBlist->remarks}}</td>
                                                        @if (auth()->check())
                                                            @if (auth()->user()->isAdmin())
                                                                <td>
                                                                    <a href="{{route('admin.companyB.edit', $companyBlist->id)}}"
                                                                       class="btn btn-primary">{{'Edit'}}</a>
                                                                </td>
                                                                <td>
                                                                    <form
                                                                        action="{{route('admin.companyB.delete', $companyBlist->id)}}"
                                                                        method="POST">
                                                                        @csrf
                                                                        @method('DELETE')
                                                ...
                                                <tr>
                                                    ...
                                                    <td></td>
                                                    <td></td>
                                                    <td>Sum of Total Hour = {{$companyBlists->sumOfTotalHour}}</td>
                                                    <td></td>
                                                </tr>
                                            @endif
                                            </tbody>
